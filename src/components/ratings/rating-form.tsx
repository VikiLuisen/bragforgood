...
export function RatingForm({ deedId, onRated }: { deedId: string; onRated: (rating: RatingWithUser) => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <div>
        <p className="text-xs font-semibold text-[var(--text-secondary)] mb-2">{t("ratings.howWasIt")}</p>
      ...
      >
        {pending ? t("ratings.submitting") : t("ratings.submit")}
      </button>
    </form>
  );
}
